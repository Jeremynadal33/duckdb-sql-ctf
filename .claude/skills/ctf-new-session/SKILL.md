...
À faire avant tout. Le repo lit ses credentials AWS + PG depuis l'env shell ou, en fallback, depuis `.env` à la racine.

> **Convention de chargement** : les variables d'environnement déjà définies dans le shell sont **prioritaires**. `.env` sert uniquement de fallback pour celles qui ne sont pas définies. Concrètement, ne **jamais** utiliser `set -a && source .env && set +a` (qui écrase l'env shell). À la place, utiliser ce snippet — à inclure dans chaque `Bash` tool call qui a besoin des creds (chaque appel est un shell neuf) :
>
> ```bash
> # Priorité aux variables d'env shell ; .env en fallback (vars non définies seulement)
...
# AWS — env shell prioritaire, .env en fallback
[ -f .env ] && while IFS='=' read -r k v; do
  [[ "$k" =~ ^[[:space:]]*# || -z "$k" ]] && continue
  [ -z "${!k+x}" ] && export "$k=$v"
done < .env
...
Si `aws sts` renvoie `ExpiredToken` ou `InvalidClientTokenId` : demander à l'utilisateur de rafraîchir son SSO (suggérer `! aws sso login --profile ctf` qu'il tape lui-même), et **attendre** sa confirmation avant de continuer. Ne pas tenter de relancer le login soi-même.
...
```bash
# env shell prioritaire, .env en fallback
[ -f .env ] && while IFS='=' read -r k v; do
  [[ "$k" =~ ^[[:space:]]*# || -z "$k" ]] && continue
  [ -z "${!k+x}" ] && export "$k=$v"
done < .env
...
### 4.2 — Historiser puis nettoyer les buckets S3

**Avant** toute suppression, proposer d'archiver la partie précédente. Le `snapshot.parquet` actuel contient le leaderboard final de la session qui vient de se terminer — une fois supprimé, c'est perdu.
...
1. Vérifier qu'un snapshot existe :
   ...
   aws s3 ls s3://duckdb-sql-ctf/leaderboard/snapshot.parquet
   ```

2. Si le fichier existe, demander à l'utilisateur (AskUserQuestion) : **"Historiser la session précédente avant suppression ?"** (Oui / Non).
   - Si le snapshot n'existe pas (première session, ou déjà nettoyé), skip cette étape.

3. Si **Oui** : demander un nom court en kebab-case décrivant la session (ex. `2026-04-bordeaux`, `2026-05-paris-mirakl`). Convention : `<YYYY-MM>-<ville>[-<contexte>]`. Puis copier :
   ```bash
   HISTORY_NAME="<nom-fourni>"  # ex: 2026-04-bordeaux
   aws s3 cp \
     s3://duckdb-sql-ctf/leaderboard/snapshot.parquet \
     s3://duckdb-sql-ctf/history/${HISTORY_NAME}.parquet
   # Vérifier la copie
   aws s3 ls s3://duckdb-sql-ctf/history/${HISTORY_NAME}.parquet
   ```
   Ne **pas** continuer vers la suppression tant que la copie n'est pas confirmée par le `ls`.

4. Supprimer les anciennes inscriptions / soumissions / leaderboard :
   ```bash
...
> **Ne pas toucher** `s3://duckdb-sql-ctf/leaderboard/answers/` (flags managés par Terraform), `s3://duckdb-sql-ctf/data/` (régénéré juste après), ni `s3://duckdb-sql-ctf/history/` (archives des sessions précédentes).
