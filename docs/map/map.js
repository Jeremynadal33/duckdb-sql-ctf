...
const LOCATIONS = [
  {
    unlockAfter: 0,
    lat: C.library?.lat ?? 48.83323, lon: C.library?.lon ?? 2.37653,
    label: 'Bibliothèque François-Mitterrand',
    sublabel: C.library?.city ?? 'Paris',
    note: 'Point de départ. Registres de prêts suspects découverts ici.',
    type: 'library',
  },
  {
    unlockAfter: 2,
    lat: C.city_hall?.lat ?? 48.86386, lon: C.city_hall?.lon ?? 2.36258,
    label: 'Hotel de Ville',
    sublabel: C.city_hall?.city ?? 'Paris',
    note: 'Mairie de Paris Centre. Registres nationaux consultés pour l\'identification du suspect',
    type: 'mairie',
  },
  {
    unlockAfter: 3,
    lat: C.quackie?.lat ?? 48.88974, lon: C.quackie?.lon ?? 2.38209,
    label: 'Domicile de Quackie Chan',
    sublabel: C.quackie?.city ?? 'Paris',
  ...
  {
    unlockAfter: 6,
    lat: C.target?.lat ?? 44.844259, lon: C.target?.lon ?? -0.576987,
    label: 'Domicile de Hugh Quackman',
    sublabel: C.target?.city ?? 'Bordeaux',
